...
#sed 's/.*\[locus_tag=\([^]]*\)\].*/>\1/g' /home/david/genome/GCF_003004765.2_ASM300476v2/GCF_003004765.2_ASM300476v2_cds_from_genomic.fna > /home/david/genome/GCF_003004765.2_ASM300476v2/GCF_003004765.2_ASM300476v2_cds_from_genomic_salmon.fna
...
rule bt2_index_genome:
    ...
    input:
        "genome/{params.genome_base}/{params.genome_base}_genomic.fna"
    output:
...
rule bt2_align:
    ...
    output:
        sam = temp("intermediate/{sra_id,\w+}.{trimmer,\w+}.bt2.sam"),
        bam = temp("intermediate/{sra_id,\w+}.{trimmer,\w+}.bt2.bam")
    threads: THREADS
    log:
...
rule bbmap_align:
    ...
    output:
        sam = temp("intermediate/{sra_id,\w+}.{trimmer,\w+}.bbmap.sam"),
        bam = temp("intermediate/{sra_id,\w+}.{trimmer,\w+}.bbmap.bam")
    threads: THREADS
    log:
...
rule htseq_count_table:
    ...
    output:
        expand("results/tables/htseq.{aligner}.{trimmer}.counts.tsv",
        trimmer = config["TRIMMER"], aligner = config["ALIGNER"])
    shadow: "minimal"
    threads: THREADS
    benchmark:
        "benchmarks/htseq.{aligner}.{trimmer}.benchmark.txt".format(trimmer=TRIMMER, aligner = ALIGNER)
    shell:
        """
...
rule feature_counts_table:
    ...
    output:
        expand("results/tables/featureCounts.{aligner}.{trimmer}.counts.tsv",
        trimmer = config["TRIMMER"], aligner = config["ALIGNER"])
    threads: THREADS
    log:
        "log/featureCounts.{aligner}.{trimmer}.log".format(trimmer=TRIMMER, aligner = ALIGNER)
    benchmark:
        "benchmarks/featureCounts.{aligner}.{trimmer}.benchmark.txt".format(trimmer=TRIMMER, aligner = ALIGNER)
    shell:
        """
